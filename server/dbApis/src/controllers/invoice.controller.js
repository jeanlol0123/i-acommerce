...
export const postInvoice  =  async (req, res) => {
    try{
        ...
        if(exist){
            const [rows] = await pool.query('INSERT INTO factura (idRelacionesEnvio,fechaGeneracion,fechaVencimiento) VALUES (?,?,?)',
...
export const updateInvoice = async(req, res) => {
    try{
        const id = req.params.id;
            ...
            const exist = await shipmentExist(idRelacionesEnvio);
            if (exist){
                const [result] = await pool.query('UPDATE factura SET idRelacionesEnvio =?, fechaGeneracion =?, fechaVencimiento =? WHERE id =?',
